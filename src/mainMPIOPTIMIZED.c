...
int main(int argc, char **argv)
{
    ...
    if (!world_rank)
    {
        ...
        QueueWork_t *queue = (QueueWork_t *)malloc(sizeof(QueueWork_t) * 100);
        if (queue == NULL)
        {
        ...
        while (start_block < combinations)
        {

            // Making it into 6-7 blocks
            unsigned long next_block = start_block + (combinations / 6);

            // Set end block max combo when next exceed it.
        ...
        while (workers_done < working_processes)
        {
            ...
            if (worker_tag == FOUND_TAG)
            {
                ...
                fprintf(fp, "PAR OPTIMIZED Guess found: %s with size: %d\n", guess, sizePass);
                fclose(fp);
            ...
            else if (worker_tag == REQUEST_WORK_TAG)
            {
                ...
                if (index > -1)
                {
                    ...
                    unsigned long start_block_s = queue[index].start;
                    unsigned long next_block_s = queue[index].end;
                    unsigned long work[2] = {start_block_s, next_block_s};
                    ...
                    MPI_Send(work, 2, MPI_UNSIGNED_LONG, worker_rank, WORK_TAG, MPI_COMM_WORLD);
                }
                ...
                else
                {
                    ...
                    if (slow_worker_rank > 0)
                    {
                        ...
                        // Slow worker start. Instead of stealing the whole start of the slow worker, we steal above half way of it.
                        // That way the worker that steals, he begins in above the middle of the slow worker work.
                        unsigned long slow_worker_start = queue[index].start + (queue[index].end - queue[index].start) / 1.60;
                        // Slow worker end
                        ...
                        // Packing it into a new buffer called work and sending it.
                        unsigned long start_block_s = slow_worker_start;
                        unsigned long next_block_s = slow_worker_end;
                        unsigned long work[2] = {start_block_s, next_block_s};
    ...
    else
    {
        ...
        while (!found)
        {
            ...
            // Receives start and end block which is packed inside a buffer called work from master. Means there might exist job, depends on the tag status.
            MPI_Recv(work, 2, MPI_UNSIGNED_LONG, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
            ...
            {
                // Receiving work from master on block range for the process.
                ...
                for (unsigned long i = start_blockr; i < end_blockr; i++)
                {
                    ...
                    MPI_Iprobe(0, EXIT_TAG, MPI_COMM_WORLD, &flag, MPI_STATUS_IGNORE);
                    ...
                    if (flag)
                    {
                        ...
                        MPI_Recv(NULL, 0, MPI_BYTE, 0, EXIT_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
